...
class Generator extends BasicGenerator {
  async writing() {
    ...
    const arrModules = [];
    for (const key in packageModules) {
      if (packageModules.hasOwnProperty(key)) {
        const version = packageModules[key] ? `@${packageModules[key]}` : '';
        arrModules.push(`${key}${version}`);
      }
    }
    strPackages = arrModules.join(' ');
