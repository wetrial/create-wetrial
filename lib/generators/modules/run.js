...
const runPublishAllGenerator = async (
  generatorPath,
  {
    name = '',
    ssubtype = '',
    cwd = process.cwd(), // 'D:\\Work\\Project\\abp\\WtFoundation.Organizations\\react',
    args = {},
  },
) => {
  return new Promise((resolve) => {
    ...
    let totalPackage = modulePackages.length;
    if (totalPackage == 0) {
      resolve(true);
    } else {
      modulePackages.map((itemPath) => {
        const Generator = require(generatorPath);
        const generator = new Generator({
          name,
          version: ssubtype,
          env: {
            cwd: path.join(cwd, itemPath),
          },
          resolved: require.resolve(generatorPath),
          args,
        });
        ...
        return generator.run(() => {
          console.log('✨ package publish Done');
          totalPackage -= 1;
          if (totalPackage === 0) {
            resolve(true);
          }
...
const run = async (config) => {
  ...
  try {
    if (subtype === 'publish-all') {
      return runPublishAllGenerator(`./publish`, config);
    } else {
