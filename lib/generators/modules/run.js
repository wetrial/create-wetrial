    ...
    const answers = await inquirer.prompt([
      {
        ...
        choices: [
          {
            name: `${f.padEnd(25)} - ${chalk.gray('安装或更新一个新的子应用模块.')}`,
            value: 'add',
            short: 'add',
          },
          {
            name: `${f.padEnd(25)} - ${chalk.gray('将已安装的子应用模块安装到宿主应用.')}`,
            value: 'install',
            short: 'install',
          },
          {
            name: `${f.padEnd(25)} - ${chalk.gray('发布当前子应用模块.')}`,
            value: 'publish',
            short: 'publish',
          },
          {
            name: `${f.padEnd(25)} - ${chalk.gray('卸载指定的子应用模块.')}`,
            value: 'remove',
            short: 'remove',
          },
        ],
